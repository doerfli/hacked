...
- `compileSdk`/`targetSdk` 37, `minSdk` 23, `jvmTarget`/source/target compatibility 17
...
Migrating from AppCompat/XML/Fragment/RecyclerView-Adapter to Compose + Material 3, dark mode support, and a new "Receding" app icon. Full plan and design rationale: `specs/ui-refactoring.md`. The plan links a Claude-generated mockup artifact (all four screens, light + dark) — fetch it via `WebFetch` on the artifact URL in that spec when aligning a screen to the mockup; the mockup HTML/CSS in that artifact is the source of truth for exact colors, spacing, and iconography (e.g. bottom nav uses Person/Lock/split-Shield glyphs, not Material's default AccountCircle/Password/Warning icons).
...
- `ui/screens/{accounts,passwords,leaks,firstuse}/` — the four Compose screens; `ui/HackedApp.kt` hosts the `NavHost` + bottom `NavigationBar`
- `ui/composable/` — shared pieces (`BreachCard`, `LabeledValue`, `HtmlLinkText`, `AppOverflowMenu`)
- `ui/fragments/SettingsFragment.kt` — the one screen staying as a Fragment (`PreferenceFragmentCompat`), hosted from `SettingsActivity`
...
- **`Modifier.fillMaxHeight()` silently collapses to zero inside an unbounded-height parent.** A `Row`/`Column` measured with unbounded height (e.g. as the content of a `Card` inside a `LazyColumn`, or any ancestor that itself wraps content) passes `Constraints.Infinity` for height to its children; `fillMaxHeight()` on a sibling then has no bound to fill against and renders at 0dp instead of erroring. This bit us on `BreachCard`'s colored status edge-bar (rendered as an invisible 4dp × 0dp sliver). `Modifier.height(IntrinsicSize.Min)` on the parent is a common fix but is only an *estimate* — it can under-measure once content wraps unpredictably (multi-line text, `FlowRow` chips), silently clipping later content (this specifically ate the Acknowledge button on some breach cards). The robust fix for a fixed-width bar/background that should match a sibling's real height is `Box` + `Modifier.matchParentSize()`, which sizes off the actual measured layout, not an intrinsic estimate. Prefer that pattern any time you need one child to match another's height inside a wrap-content parent.
